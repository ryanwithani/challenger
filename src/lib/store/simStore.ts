...
interface SimState {
    ...
    assignToChallenge: (simId: string, challengeId: string) => Promise<void>
    unassignFromChallenge: (simId: string) => Promise<void>
...
export const useSimStore = create<SimState>((set, get) => ({
    ...
    assignToChallenge: async (simId: string, challengeId: string) => {
        set({ loading: true, error: null })
        const supabase = createSupabaseBrowserClient()
        try {
            const { data, error } = await supabase
            ...
            set({
                currentSim: data,
                familyMembers: get().familyMembers.map(s => s.id === simId ? data : s),
                loading: false,
            })
        } catch (error: any) {
            set({ error: error.message, loading: false })
    ...
    unassignFromChallenge: async (simId: string) => {
        set({ loading: true, error: null })
        const supabase = createSupabaseBrowserClient()
        try {
            const { data, error } = await supabase
